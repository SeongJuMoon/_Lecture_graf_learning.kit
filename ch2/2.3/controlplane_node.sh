...
# make rerepo-graf_learning.kit and put permission
cat <<EOF > /usr/local/bin/rerepo-graf_learning.kit
#!/usr/bin/env bash
rm -rf $HOME/_Lecture_graf_learning.kit 
git clone https://github.com/seongjumoon/_Lecture_graf_learning.kit.git $HOME/_Lecture_graf_learning.kit
find $HOME/_Lecture_graf_learning.kit -regex ".*\.\(sh\)" -exec chmod 700 {} \;
EOF
chmod 700 /usr/local/bin/rerepo-graf_learning.kit
